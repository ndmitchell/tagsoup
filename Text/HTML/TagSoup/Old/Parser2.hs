...
Move Char to CharT, and do it with bitfields and fast lookup
-}


class Str string where
    nullStr :: string -> Bool
    appendStr :: string -> string -> string


class Str string => P state string where
    -- uncons, standard
	uncons :: state -> Maybe (Char, state)
    empty :: state -> Bool
    
	breakStr :: String -> state -> (string, state)
    spanF :: (Char -> Bool) -> state -> (string, state)

    warning :: String -> [Tag string] -> [Tag string]
    position :: [Tag string] -> [Tag string]


warningIf :: Bool -> String -> [Tag string] -> [Tag string]
warningIf True  = warning
warningIf False = flip const


tags :: P state string => state -> [Tag string]
tags x0 =
...
-- Continuation based string
data CString str = CString
    {result :: Char -> CString str
    ,append :: str -> str
    }
...
cstring s = CString s (s++) (

{-

tag = comment | close | open | text

open = openname 


{-
IDEA: Have a 2-level representation!

Close name = </name>
Open name = <name
Att name value = name=value
AttEnd = >
Text (cont string) = string -> string -> string
Comment text = <!--text-->


tag = *{comment close entity open text WARNING}

comment(*TagComment) = "<!--" _$1 "-->"
close(*TagClose) = "</" name$1 _#empty ">"
open(*TagOpen) = "<" name$1 spaces *att$2 ">"
text(*TagText) = _$1 !/<&/

space = /:isSpace/
spaces = *space

name = /:isNameChar1/ */:isNameChar2/

null = *//
att(*(,)) = name$1 *space {value null}$2
value = "=" *space val$1 *space
val = {dquote squote str}
dquote = "\"" entitytext "\""
...
ANSWER: MUST BE FULLY GENERAL
Lazy parser generator
...
tag $"<!-- $rest" = comment rest
tag $"</ $rest" = close rest
tag $"<? $rest" = open True rest
tag $"< $rest" = open False rest
tag x = text

comment $"$xs(.*) $close(-->) $rest" =
    (:) TagComment xs $
    warningIf (null close) "Unexpected end when looking for -->" $
    tags rest

close $"$xs(#name) #space* $junk(.*) $close(>) $rest" =
    (:) TagClose xs $
    warningIf (not $ null junk) "Junk tag in closing tag" $
    tags rest

open question $"$xs(#name) #space* $rest" =



comment
    "<!--" $xs".*" $ys"-->"
    [TagComment $xs, warningIfNull ys "Looking for -->"]
...
tags
    ...
    | "</" = close ; tags
    | "&" = entity ; tags
    | "<?" = open ; atts; qattend ; tags
    | "<!" = open ; atts; attend ; tags
    | "<" = open ; atts; attend ; tags
    | "" = text ; tags
...
comment =
    $1 = break "-->"
    tag TagComment $1
    warningIf empty "Unexpected end while looking for -->"
...
close =
    ...
    $3 = break ">"
    tag TagClose $1
    warningIf (not $ null $3) "Junk in closing tag"
    warningIf empty "Unexpected end when looking for >"

name = regex $ isNameCharFirst . isNameCharRest*
spaces = span (== ' ')
...
text :: P state string => state -> [Tag string]
text x = tagText str $ tags x2
    where (str, x2) = spanF (`notElem` ">&")


tagText :: P state string => string -> [Tag string] -> [Tag string]
tagText x xs
    | nullStr x = xs
    | otherwise = TagText (x `appendStr` y) : zs
        where
            (y,zs) = case xs of
                        TagText y:zs -> (y,zs)
                        zs -> (nullStr, zs)
...
comment :: P state string => state -> [Tag string]
comment x = (:) TagComment str $
            warningIf (empty x2) "Unexpected end while looking for \"-->\" in comment" $
            tags x2
    where (str, x2) = breakStr "-->" x


close :: P state string => state -> [Tag string]
close x1 = let
    (n,x2) = name x1
    (bad,x3) = spanF (/= '>') (spaces x2)
    in (:) TagClose n $
       warningIf (not $ nullStr bad) "Junk in closing tag" $
       warningIf (empty x4) "Unexpected end when looking for \">\"" $
       tags x4


name :: P state string => state -> (string, state)
name x = case uncons x of
              Just (c,x) | isNameCharFirst x ->
                    let (a,b) = spanF isNameChar x
                    in (c `cons` a
